---
name: purify
description: Purify a spec via AISP 5.1 — generate AISP from implicit context or a referenced spec, ask clarifying questions if needed, then update the source in plain English.
---
...
When the gate passes: update the content that was translated to AISP from the AISP document. Make the updates by finding differences between the AISP and the source. The goal is to "purify" the source with the AISP transformation.
